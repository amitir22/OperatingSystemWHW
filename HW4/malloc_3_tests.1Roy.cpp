...
int main() {
    ...
    // sfree of b3 will combine two sfree blocks back together.
    sfree(b3);
    assert(_num_free_blocks() == 1);
    assert(_num_free_bytes() == 3000);
    assert(_num_allocated_blocks() == 6);
    ...
    // checking if sfree combine works on the other way.
    // (a a f a a a /sfree b4/ a a f f a a /combine b3 and b4/ a a f a a)
    sfree(b4);
    assert(_num_free_blocks() == 1);
    ...
    // the sfree block is of size 7000 + META_SIZE, so the splittable part is 127.
    b3 = smalloc(7000 - 127);
    assert(_num_free_blocks() == 0);
    assert(_num_free_bytes() == 0);
    assert(_num_allocated_blocks() == 5);
