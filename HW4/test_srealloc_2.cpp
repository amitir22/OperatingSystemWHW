...
//Reuse a memory block if the size requested is smaller than the old block
    ...
    //Check that oldp is not freed if srealloc fails
    ...
    ********************************************/
    ...
    //Test srealloc for a large block
    ...
    //Should create a new block and sfree the old one - without wilderness increase
    ...
    //Check that failure for large block srealloc dosen't make any changes (no sfree)
...
void remalloc_3_test() {
    ...
    ********************************************/
    ...
    //Check that failure for large block srealloc dosen't make any changes (Without uniting any blocks)
    int* failed_realloc = (int*)srealloc(large_malloc, 0);
    assert(!failed_realloc);
    assert(large_malloc);
    for (int i = 0; i < 40; i++) {
